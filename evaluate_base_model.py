...
OUT_PATH = f"base_model_results.jsonl"
...
os.makedirs("images_base_eval", exist_ok=True)
...
with open(OUT_PATH, "w") as fout:
    for i, (question, image, true_ans) in enumerate(tqdm(zip(qs, imgs, trues), total=len(qs))):
        ...
        image_pil.save(f"images_base_eval/image_{i}_original.png")
        
        # Resize image to 224x224 (LLaVA's expected size)
        image_pil = image_pil.resize((224, 224), Image.Resampling.LANCZOS)
        
        # Save the resized image
        image_pil.save(f"images_base_eval/image_{i}_resized.png")
